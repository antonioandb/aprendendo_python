# set é um conjunto que não permite repetições, ele não pussuem indices a posição na memoria é arbitrária.
...
# remove: semelhante ao discard mas gera erro caso o item não existir.
...
# clear: faz a limpeza do set removendo tudo
frutas.clear()
print(frutas)
...
# também é possível usar o operador | (pipe)
união = set1 | set2 
...
# também é possível usar o operador & (ampersand)
...
# também é possível usar o operador - (minus)
diferença = set1 - set2
...
# também é possível usar o operador ^ (caret)
diferente = set1 ^ set2 
